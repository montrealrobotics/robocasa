...
Usage:
    ...
    uv run -m robocasa.scripts.regenerate_robocasa_dataset \
...
def main(args):
    ...
    for file_idx, hdf5_file in enumerate(hdf5_files):
        ...
        replayed_demos_counter = (
            0  # Counter for demos that were replayed (excludes only reset timeouts)
        )
        ...
        if args.verbose:
            print(
                colored(
                    f"Initializing environment for {env_kwargs['env_name']}...",
                    "yellow",
                )
            )
        ...
        if args.data_collection:
            rollout_data_subdir = os.path.join(
                rollout_data_dir, os.path.dirname(rel_path)
            )
        ...
        for ind in range(len(demos)):
            ...
            try:
                ...
                print(
                    colored(
                        f"Timeout during reset for episode {ep}, skipping to next episode",
                        "yellow",
                    )
                )
                ...
                episode_log[
                    "reason"
                ] = f"Reset operation timed out after 5 seconds: {str(e)}"
                ...
                try:
                    ...
                    print(
                        colored(
                            f"Failed to recreate environment: {env_create_error}", "red"
                        )
                    )
            ...
            if num_actions_replayed < 5:
                print(
                    colored(
                        f"  Skipping episode: less than 5 actions to replay (only {num_actions_replayed}).",
                        "yellow",
                    )
                )
            ...
            if args.data_collection and len(primary_images_list) > 0:
                ...
                collected_data = dict(
                    primary_images=np.stack(
                        primary_images_list, axis=0
                    ),  # (T, H, W, C) - left camera
                    secondary_images=np.stack(
                        secondary_images_list, axis=0
                    ),  # (T, H, W, C) - right camera
                    ...
                    success=bool(
                        success
                    ),  # True for successful episodes, False for unsuccessful
                )
                        ...
                        for k, v in collected_data.items():
                            if isinstance(v, np.ndarray):
                                ...
                                if is_image and args.jpeg_compress:
                                    jpeg_list = [
                                        jpeg_encode_image(frame, quality=95)
                                        for frame in v
                                    ]
                                    dt = h5py.vlen_dtype(np.dtype("uint8"))
                                    f.create_dataset(
                                        k + "_jpeg", data=jpeg_list, dtype=dt
                                    )
                                else:
                ...
                if args.verbose:
                    print(
                        f"  Saved rollout episode data: episode={global_episode_counter}, success={success}"
                    )
            ...
            if success:
                ...
                obs_grp.create_dataset(
                    ...
                    data=np.stack(agentview_left_images, axis=0),
                )
                obs_grp.create_dataset(
                    "robot0_agentview_right_rgb",
                    data=np.stack(agentview_right_images, axis=0),
                )
                obs_grp.create_dataset(
                    "robot0_eye_in_hand_rgb", data=np.stack(eye_in_hand_images, axis=0)
                )
            ...
            if len(agentview_left_images) > 0:
                ...
                for img_left, img_right, img_wrist in zip(
                    ...
                    combined_img = np.concatenate(
                        [img_left, img_right, img_wrist], axis=1
                    )
            ...
            if args.debug:
                ...
                summary_log_path = os.path.join(
                    args.target_dir, "regeneration_summary.json"
                )
                ...
                episodes_log_path = os.path.join(
                    args.target_dir, "regeneration_episodes.json"
                )
                ...
                if args.data_collection:
                    ...
                    print(
                        f"JPEG compression: {'Enabled' if args.jpeg_compress else 'Disabled'}"
                    )
            ...
            if args.num_demos is not None and replayed_demos_counter >= args.num_demos:
                print(
                    colored(
                        f"\nReached max number of demos ({args.num_demos}), stopping processing.",
                        "cyan",
                    )
                )
                break
    ...
    stats["end_time"] = time.time()
    stats["elapsed_time_seconds"] = stats["end_time"] - stats["start_time"]
    stats["elapsed_time_formatted"] = str(
        datetime.timedelta(seconds=int(stats["elapsed_time_seconds"]))
    )
    ...
    if args.data_collection:
        ...
        print(
            f"[COMPRESSION] JPEG compression: {'Enabled' if args.jpeg_compress else 'Disabled'}"
        )
